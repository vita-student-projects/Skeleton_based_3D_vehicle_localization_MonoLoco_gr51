...
class PreprocessKitti:
    ...
    categories_gt = dict(train=['car'], val=['car'])
    ...
    def run(self):
        ...
        for self.name in self.names_gt:
            ...
            category = 'all' if self.phase == 'train' else 'car'
            if file_not_found:
                self.stats['fnf'] += 1
                continue
    ...
    def _cout(self):
        ...
        print(f"Files with at least one car: {self.stats['gt_files_ped']}")
